...
public class GPFile implements Serializable
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6683014100132308894L;
	private long size;
    ...
    public GPFile(String path, long size, long modificationTime)
    {
        this(path, size, modificationTime, false);
    }
    
    public GPFile(String path, long size, long modificationTime, boolean isDirectory)
    {
        path = path.replace('\\', '/');
        if(path.endsWith("/"))
        {
    ...
    public GPFile(String path, boolean isDirectory)
    {
